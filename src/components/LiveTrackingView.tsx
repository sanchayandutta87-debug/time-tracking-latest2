...
export default function LiveTrackingView() {
  ...
  const fetchLiveTrackingData = async () => {
    ...
    try {
      ...
      const [userRes, attendanceRes, activeRes, tasksRes] = await Promise.all([
        supabase.from('users').select('*'),
        supabase.from('attendance').select('*').eq('date', new Date().toISOString().split('T')[0]),
        supabase.from('time_entries').select('*, projects(name), tasks(name)').is('end_time', null),
        supabase.from('tasks').select('assignee_id, projects(name), name')
      ]);
      ...
      const activeEntries = activeRes.data || [];
  ...
  const handleTakeScreenshot = async (userId: string, userName: string) => {
    ...
    try {
      ...
      showToast(`📸 Screenshot captured for ${userName}! It has been saved and logged to their activity.`);
    } catch (error) {
  ...
  return (
    <div className="p-8 bg-transparent min-h-full">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        <div className="bg-white dark:bg-black flex items-center gap-2 border border-gray-100 dark:border-gray-800 rounded-lg px-3 py-2.5 w-full md:w-80 shadow-sm">
        ...
        <div className="relative w-full md:w-48">
          <input 
            ...
            disabled
            className="w-full border border-gray-100 dark:border-gray-800 rounded-lg py-2.5 px-4 text-sm outline-none bg-gray-50 dark:bg-black shadow-sm pr-10 cursor-not-allowed dark:text-gray-400"
          />
      ...
      {/* Table Container */}
      <div className="bg-white dark:bg-black rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden min-h-[400px] flex flex-col">
        {isLoading ? (
